	...
	if (isset($_POST['nheure']) or isset($_POST['matr']) or isset($_GET['modif_dev']) or isset($_GET['appelj'])) {
		...
		if (!isset($_POST['matr'])) {
			...
			if (isset($_GET['modif_dev'])) {
				$_SESSION['nheurej']=$_GET['modif_dev'];
			}
		...
		$hdebut=$_SESSION['hdebutj'];
		$nheure=$_SESSION['nheurej'];
		$dateabs = date('Y-m-d');
		...
		if (empty($prodeleve)){ 
			...
			<table class="payement" style=" margin-left: 10px; width: 100%;">
				...
				$prodpres=$DB->querys('SELECT  count(matricule) as nbrepres from inscription where nomgr=:nom', array('nom'=>$_SESSION['groupe']));
