    ...
    <div class="container-fluid">
		...
		<div class="col-sm-12 col-md-10"><?php
			...
			if (isset($_GET['supidabs'])) {
				...
				$DB->delete('DELETE FROM accesiteexclus WHERE id = ?', array($_GET['supidabs']));?>
			...
			if (isset($_POST['j1'])) {
                ...
                $prodeleve=$DB->query("SELECT *from inscription  inner join accesiteexclus on matricule = mat_abs where nomgr='{$_SESSION['classe']}' and promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteexclus.id not in(SELECT id_absence FROM justabsence)  order by (mat_abs)");

			}elseif (isset($_GET['absmatri'])) {

				// $prodeleve=$DB->query('SELECT accesiteexclus.id as id, nomel, prenomel, eleve.adresse as adresse, eleve.sexe as sexe, pere, mere, eleve.naissance as naissance, eleve.matricule as matricule, phone, nommat, nomen, prenomen, nbreheure, hdebut, date_format(date_abs,\'%d/%m/%Y \') as date_abs, nomgr as classe, nomf from eleve inner join absence on eleve.matricule=absence.matricule inner join matiere on matiere.codem=absence.codem inner join enseignant on enseignant.matricule=absence.codens inner join contact on eleve.matricule=contact.matricule inner join formation on formation.codef=matiere.codef where promo=:promo and absence.matricule=:matr and semestre=:sem and accesiteexclus.id not in(SELECT id_absence FROM justabsence)', array('promo'=>$_GET['promo'], 'matr' => $_GET['absmatri'], 'sem'=>$_GET['sem']));

                // $prodeleve=$DB->query("SELECT *from accesiteexclus inner join inscription on matricule = mat_abs where promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteexclus.id not in(SELECT id_absence FROM justabsence)  order by (mat_abs)");

			}else{

				$prodeleve=$DB->query("SELECT *from inscription  inner join accesiteexclus  on matricule = mat_abs where promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteexclus.id not in(SELECT id_absence FROM justabsence)  order by (mat_abs)");
			} ?>
