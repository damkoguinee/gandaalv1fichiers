...
<page backtop="5mm" backleft="3mm" backright="1mm" backbottom="5mm" footer="page;"><?php
  ...
  if (isset($_GET['enseig'])) {
    ...
    if(!empty($_SESSION['niveauf'])) {
      ...
      $prodm=$DB->query('SELECT  *from enseignant inner join contact on enseignant.matricule=contact.matricule inner join login on login.matricule=enseignant.matricule inner join niveau on niveau.matricule=enseignant.matricule where nom=:niv order by(prenomen)', array('niv'=>$_SESSION['niveauf']));

        if($_SESSION['niveauf']=='lycee'){
          $niveau='du Secondaire';
        }elseif($_SESSION['niveauf']=='college'){
          $niveau='du Secondaire';
        }elseif($_SESSION['niveauf']=='primaire'){
          $niveau='du Primaire';
        }else{
      ...
      $prodm=$DB->query('SELECT  *from enseignant inner join contact on enseignant.matricule=contact.matricule inner join login on login.matricule=enseignant.matricule order by(prenomen)');

    }?>
      
    <table class="tablistebul" style="width: 100%;">
      <thead>
        <tr><th colspan="6">Liste des Enseignants <?=$niveau;?></th></tr>
  ...
  if (isset($_GET['perso'])) {
    ...
    $prodm=$DB->query('SELECT  *from personnel inner join contact on numpers=contact.matricule inner join login on login.matricule=numpers order by(prenom)');?>
  ...
  if (isset($_GET['groupe'])) {
    ...
    <table class="tablistebul" style="width: 90%;">
      ...
      <thead>
        ...
        <tr><th colspan="3">Liste des Classes</th></tr>

        <tr>
          <th height="25">Classe</th>
          <th>Formation</th>
          <th>Coordinateur</th>
        </tr>
          ...
          foreach ($prodm as $formation) {?>
            ...
            <tr>
              ...
              if ($formation->classe=='1') {?>
                ...
                <td><?=' '.$formation->classe.'1ère année '.$formation->nomf;?></td><?php
  ...
  if (isset($_GET['listel'])) {
    ...
    <table class="tablistel">
      ...
      <thead>
        ...
        <tr><th colspan="8">Liste des <?=$_SESSION['typeel'];?> Inscrits en <?=$_SESSION['promo'];?></th></tr>
        ...
        if (empty($prodeleve)) {
          ...
          foreach ($prodeleve as $key=> $eleve) {?>
            ...
            <tr>
              ...
              <td><?=ucwords($eleve->pere).' et '.ucwords(strtolower($eleve->mere));?></td><?php
              ...
              if ($eleve->classe==1) {?>
                ...
                <td height="20"><?=$eleve->classe.'ème '.$eleve->nomf;?></td><?php                
              }?>
  ...
  if (isset($_GET['voir_e'])) {
    ...
    <table class="tablistel">
      ...
      <thead>
        ...
        <tr><th colspan="8">Liste des <?=$_SESSION['typeel'];?> <?=$classe;?></th></tr>
      ...
      # code...
          ...
          foreach ($prodm as $key=> $formation) {?>
            ...
            <tr>
              ...
              <td><?=strtoupper($formation->pere).' et de '.ucfirst(strtolower($formation->mere));?></td>
...
// Afficher les eleves d'un niveau
  ...
  if (isset($_GET['voir_cursus'])) {
    ...
    <table class="tablistel">
      ...
      <thead>
        ...
        <tr><th colspan="8">Liste des <?=$_SESSION['typeel'];?> niveau <?=$_GET['voir_cursus'];?></th></tr>
  ...
  if (isset($_GET['voir_elg'])) {
    ...
    <table class="tablistel">
      ...
      <thead>
        ...
        <tr><th colspan="7">Liste des <?=$_SESSION['typeel'];?> du <?=$prodf['nomgr'];?></th></tr>
  ...
  if (isset($_GET['numfac'])) {
    ...
    <table class="border" style=" margin: auto; margin-top: 30px;">
      ...
      <thead>
        <tr>
          <th colspan="4" style="border: 0px; border-right: 1px; text-align: left; font-size: 12px;">NB: Aucun remboursement ou transfert n'est possible après paiement.</th>

          <th colspan="2">N° de Compte Ecobank</th>
        </tr>
        <tr>
          <th colspan="4" style="border: 0px; border-bottom: 1px; border-right: 1px; text-align: left; font-size: 12px;">Le paiement de toute scolarité entamée est entièrement dû.</th>

          <th colspan="2"><?=$etab['cbanque'];?></th>
        </tr>

        <tr>
          <th colspan="6" style="border-bottom: 0px; text-align: center;">Réçu de Payement des frais de scolarité de la <?=$_GET['tranche'];?></th>
        </tr>
        <tr>
          <th colspan="2" height="10" style="border-right: 0px;">Année-Scolaire: <?=($_SESSION['promo']-1).'-'.$_SESSION['promo'];?></th>

          <th style="border-right: 0px;">Date: <?=$_GET['date'];?></th>
          <th style="border-right: 0px;">N° Réçu: <?=$_GET['numfac'];?></th>
          <th colspan="2">Type de Payement: <?=$_GET['type'];?></th>
        </tr>
      ...
      <tbody><?php
          ...
          foreach ($products as $product){
            ...
            $prodtot=$DB->querys('SELECT sum(montant) as montant FROM histopayefrais WHERE matricule= ? and promo=? and tranche=?', array($product->matricule, $_SESSION['promo'], $_GET['tranche']));
            ...
            $prodscol=$DB->querys('SELECT sum(montant) as montant from scolarite where codef=:code and tranche=:tranche and promo=:promo', array('code'=>$product->codef, 'tranche'=>$_GET['tranche'], 'promo'=>$_SESSION['promo']));

            $montp+=$product->montant;
            $totp+=$prodtot['montant'];
            $restp+=$prodscol['montant']-$prodtot['montant'];?>
            ...
            <tr>
              ...
              <td style="text-align: right;"><?=number_format($prodtot['montant'],0,',',' '); ?></td>
              <td style="text-align: right;"><?=number_format($prodscol['montant']-$prodtot['montant'],0,',',' '); ?></td>
      ...
      if (sizeof($products)>1) {?>
        ...
        <tfoot>
          <tr>
            ...
            <th style="text-align: right;"><?=number_format($totp,0,',',' ');?></th>
            <th style="text-align: right;"><?=number_format($restp,0,',',' ');?></th>
          </tr>
        </tfoot><?php
      }?>
  ...
  if (isset($_GET['paytotemp'])) {
    ...
    <table class="tablistebul">
      <thead>
          <tr><th colspan="4">Salaires payés de <?=$_GET['nomel'];?></th></tr>
...
if (isset($_GET['synthesea']) or isset($_GET['synthesem']) or isset($_GET['synthesej']) or isset($_GET['syntheseg'])) {?>
  ...
  <table class="tablistebul">
    <thead>
      <tr><th colspan="4" height="25"><?='Comptabilité générale de '.$_GET['annee'];?></th></tr>
    ...
    <tbody><?php
      ...
      if (isset($_GET['synthesea'])) {
        ...
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(motif) AS nbre, motif FROM payement WHERE motif=:type AND DATE_FORMAT(datepaye, \'%Y\')=:annee', array('type'=>'inscription','annee' => $_GET['synthesea']));

      }elseif (isset($_GET['synthesem'])) {
        
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(motif) AS nbre, motif FROM payement WHERE motif=:type AND  DATE_FORMAT(datepaye, \'%m/%Y\')=:annee', array('type'=>'inscription','annee' => $_GET['synthesem']));

      }elseif (isset($_GET['synthesej'])) {
        
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(motif) AS nbre, motif FROM payement WHERE motif=:type AND DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('type'=>'inscription','annee' => $_GET['synthesej']));
      }else{

        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(motif) AS nbre, motif FROM payement WHERE motif=:type', array('type'=>'inscription'));
      }
      ...
      foreach( $products as $product ) {
        ...
        <tr><?php
          if (empty($product->motif)) {
              ...
              <td height="20">Frais d'inscription</td>
      ...
      if (isset($_GET['synthesea'])) {
        ...
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(tranche) AS nbre, tranche FROM payementfraiscol WHERE  DATE_FORMAT(datepaye, \'%Y\')=:annee', array('annee' => $_GET['synthesea']));

      }elseif (isset($_GET['synthesem'])) {
        
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(tranche) AS nbre, tranche FROM payementfraiscol where DATE_FORMAT(datepaye, \'%m/%Y\')=:annee', array('annee' => $_GET['synthesem']));

      }elseif (isset($_GET['synthesej'])) {
        
        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(tranche) AS nbre, tranche FROM payementfraiscol where DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('annee' => $_GET['synthesej']));

      }else{

        $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(tranche) AS nbre, tranche FROM payementfraiscol');
      }
      ...
      foreach ($typedoc as $keydec => $document){
        ...
        if (isset($_GET['synthesea'])) {
          ...
          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre, motif FROM decaissement WHERE motif=:type', array('type'=>$document));
      ...
      if (isset($_GET['synthesea'])) {
          ...
          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payepersonnel WHERE DATE_FORMAT(datepaye, \'%Y\')=:annee', array('annee' => $_GET['synthesea']));

      }elseif (isset($_GET['synthesem'])) {

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payepersonnel WHERE DATE_FORMAT(datepaye, \'%m/%Y\')=:annee', array('annee' => $_GET['synthesem']));

      }elseif (isset($_GET['synthesej'])){

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payepersonnel WHERE DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('annee' => $_GET['synthesej']));

      }else{

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payepersonnel WHERE DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('annee' => date('Y-m-d')));
      }
      ...
      if (isset($_GET['synthesea'])) {
          ...
          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payenseignant WHERE DATE_FORMAT(datepaye, \'%Y\')=:annee', array('annee' => $_GET['synthesea']));

      }elseif (isset($_GET['synthesem'])) {

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payenseignant WHERE DATE_FORMAT(datepaye, \'%m/%Y\')=:annee', array('annee' => $_GET['synthesem']));

      }elseif (isset($_GET['synthesej'])){

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payenseignant WHERE DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('annee' => $_GET['synthesej']));

      }else{

          $products =$DB->query('SELECT SUM(montant) AS montant, COUNT(id) AS nbre FROM payenseignant WHERE DATE_FORMAT(datepaye, \'%Y-%m-%d\')=:annee', array('annee' => date('Y-m-d')));
      }
      ...
      $nbretotal=$nbreins+$nbrefrais+$nbresortie+$nbresortiep+$nbresortiens;
      $totalcredit=$totins+$totfrais;
      $totaldebiter=$sortie+$sortiep+$sortiens;
...
if (isset($_GET['deca']) or isset($_GET['decm']) or isset($_GET['decg'])) {?>
  ...
  <table class="tablistel">
    <thead>
      <tr><th colspan="10" height="25"><?='Liste des élèves en découvert '.$_GET['annee'];?></th></tr>
    ...
    <tbody><?php
        ...
        if (isset($_GET['deca'])) {
         ...
         $prodpaye =$DB->query('SELECT inscription.matricule as matricule, nomel, prenomel, adresse, sexe, pere, mere, DATE_FORMAT(naissance, \'%d/%m/%Y\') AS naissance, codef FROM eleve inner join inscription on inscription.matricule=eleve.matricule WHERE inscription.annee=:promoins and inscription.matricule not in(SELECT matricule FROM payementfraiscol WHERE promo=:annee) order by(prenomel)', array('promoins'=>$_GET['deca'], 'annee' => $_GET['deca']));

        }elseif (isset($_GET['decm'])) {
          
          $prodpaye =$DB->query('SELECT inscription.matricule as matricule, nomel, prenomel, adresse, sexe, pere, mere, DATE_FORMAT(naissance, \'%d/%m/%Y\') AS naissance, codef FROM eleve inner join inscription on inscription.matricule=eleve.matricule WHERE inscription.annee=:promoins and inscription.matricule not in(SELECT matricule FROM payementfraiscol WHERE (montant>=:montant and promo=:annee and tranche=:mois)) order by(prenomel)', array('promoins'=>$_SESSION['annee'], 'montant'=>$_SESSION['montantscol'], 'annee' => $_SESSION['annee'], 'mois'=>$_GET['decm']));

        }elseif (isset($_GET['decg'])) {
          
          $prodpaye =$DB->query('SELECT eleve.matricule as matricule, nomel, prenomel, adresse, sexe, pere, mere, DATE_FORMAT(naissance, \'%d/%m/%Y\') AS naissance, codef FROM eleve inner join inscription on eleve.matricule=inscription.matricule WHERE nomgr=:nom and inscription.annee=:promoins and eleve.matricule not in(SELECT matricule FROM payementfraiscol WHERE(montant>=:montant and promo=:annee and tranche=:mois)) order by(prenomel)', array('promoins'=>$_SESSION['annee'], 'montant'=>$_SESSION['montantscol'], 'annee' => $_SESSION['annee'], 'mois'=>$_SESSION['mensuellec'], 'nom'=>$_GET['decg']));
        }
        ...
        foreach($prodpaye as $key=>$payeloc ){
          ...
          $prodscol = $DB->querys('SELECT montant FROM scolarite WHERE promo=:promo and codef=:code', array('promo'=>$_SESSION['promo'], 'code'=>$payeloc->codef));

          $montantscol=$prodscol['montant'];

          $prodcredit =$DB->query('SELECT montant, remise FROM payementfraiscol inner join inscription on inscription.matricule=payementfraiscol.matricule WHERE promo=:promo and annee=:promoins and payementfraiscol.matricule=:mat and tranche=:mois', array('promo'=>$_SESSION['annee'], 'promoins'=>$_SESSION['promo'], 'mat' => $payeloc->matricule, 'mois'=>$_SESSION['mensuellec']));
          ...
          if (empty($prodcredit)) {
            ...
            foreach ($prodcredit as $montant) {
              ...
              $resterem=$montantscol-(($montant->montant+(($montant->remise/100)*$montantscol)));
              $reste2+=$resterem;?>
...
if (isset($_GET['courrier'])) {?>
  ...
  <div style="width:700px; margin-left: 50px;">
    ...
    <p>Sauf erreur de notre part, votre fils/fille <strong><?=$_GET['nom'];?></strong>, né(e) en <strong><?=$_GET['annee'];?></strong> inscrit(e) en <strong><?=$_GET['inscrit'];?></strong> n'est pas à jour dans ses frais de scolarité concernant la <strong><?=$_GET['tranche'];?></strong>. La date limite de payement de la <strong><?=$_GET['tranche'];?></strong> était le <strong><?=$_GET['date'];?></strong></p>
