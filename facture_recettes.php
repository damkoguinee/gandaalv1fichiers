...
<page backtop="5mm" backleft="3mm" backright="1mm" backbottom="5mm"><?php
  ...
  if (isset($_GET['matricule'])) {
    ...
    $products= $DB->query("SELECT *FROM versement WHERE nom_client='{$_GET['matricule']}' and promo='{$_SESSION['promo']}' order by(versement.id)");

    $eleve=$panier->nomElevex($_GET['matricule']);
    if (empty($eleve)) {
        $designation="Nom & Prénom :";
    ...
    $design_classe="Classe :";
    
    if ($_GET['matricule']=="autres") {
        $design_classe="Classe";
        $designation="Autres";
        $classe="";
        $eleve="" ;
    ...
    <table class="border" style=" margin: auto; margin-top: 30px;">
      <thead>
        <tr>
          <th colspan="7" style="font-size: 12px; text-align: left;">NB: Aucun remboursement ou transfert n'est possible après paiement. Le paiement de toute activité entamée est entièrement dû.</th>
        </tr>

        <tr>
          <th colspan="7" style="border-bottom: 0px; text-align: center;">Réçu de Payement / Année-Scolaire: <?=($_SESSION['promo']-1).'-'.$_SESSION['promo'];?></th>
        </tr>

        <tr>
          <th colspan="7" height="10" style=" text-align: left;">Matricule: <?=$_GET['matricule'];?> / <?=$designation;?> <?=$eleve;?> / <?=$design_classe;?> <?=$classe;?> / Date: <?=date("d/m/Y");?></th>
        </tr>
      ...
      <tbody><?php
          ...
          foreach ($products as $key=> $product){
            ...
            $totp+=$product->montant;
            $dated=(new DateTime($product->date_versement))->format("d/m/Y");
            $now = date("d/m/Y"); 
            if ($now==$dated) {
                $totnow +=$product->montant;
            } ?>
            <tr>
              <td><?=$key+1;?></td>
              <td><?=$dated;?></td>
              <td><?=ucfirst(strtolower($product->motif));?></td>
              <td style="text-align: right;"><?=number_format($product->montant,0,',',' '); ?></td>
              <td><?=$product->type_versement;?></td>
              <td><?=$product->numcheque;?></td>
              <td><?=$product->banquecheque;?></td>
      ...
      if (sizeof($products)>1) {?>
        ...
        <tfoot>
          <tr>
            <th colspan="4">Dernier(s) Paiement(s): <?=number_format($totnow,0,',',' ');?></th>
            <th colspan="3">Total: <?=number_format($totp,0,',',' ');?></th>
          </tr>
        </tfoot><?php
      }?>
