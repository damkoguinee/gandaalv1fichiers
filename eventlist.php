        ...
        <div class="col" style="overflow: auto;"><?php 
            ...
            <table class="table table-hover table-bordered table-striped table-responsive text-center">
                <thead>
                    ...
                    <tr>
                        ...
                        <th colspan="4">
                            ...
                            <form class="form" method="POST" action="eventlist.php" id="suitec" name="termc">
                        ...
                        <th colspan="7">
                            <!-- <a class="btn btn-info" href="printdoc.php?horairep" target="_blank"><img  style="height: 20px; width: 20px;" src="css/img/pdf.jpg"></a> -->
                ...
                if (isset($_POST['termec'])) {
                  ...
                  $prodm=$DB->query('SELECT  DISTINCT(codensp), events.id as id, codemp, nommat, nomgrp, nomen, prenomen, codensp, name, debut, fin, lieu, phone, joursEvent FROM events inner join matiere on codemp=codem inner join enseignant on matricule=codensp inner join contact on enseignant.matricule=contact.matricule  WHERE events.id not in(SELECT idevent FROM horairet where date_format(datet,\'%Y-%m-%d \')=?) and date_format(debut,\'%Y-%m-%d\') LIKE? and(enseignant.matricule LIKE? or nomen LIKE ? or prenomen LIKE ? or phone LIKE ?) order by(prenomen) LIMIT 10',array($_SESSION['moisp'], $_SESSION['moisp'],"%".$terme."%", "%".$terme."%", "%".$terme."%", "%".$terme."%"));
                  
                }elseif (isset($_POST['mois'])) {

                    $prodm=$DB->query('SELECT  DISTINCT(codensp), events.id as id, codemp, nommat, nomgrp, nomen, prenomen, codensp, name, debut, fin, lieu, phone, joursEvent FROM events inner join matiere on codemp=codem inner join enseignant on matricule=codensp inner join contact on enseignant.matricule=contact.matricule  WHERE events.id not in(SELECT idevent FROM horairet where date_format(datet,\'%Y-%m-%d \')=:debuth) and date_format(debut,\'%Y-%m-%d \')=:debut and events.promo=:promo order by(prenomen) LIMIT 10', array('debuth'=>$_POST['mois'], 'debut'=>$_POST['mois'], 'promo'=>$_SESSION['promo']));
                    ...
                    if (!empty($_SESSION['search'])) {
                        ...
                        $prodm=$DB->query('SELECT DISTINCT(codensp), events.id as id,  codemp, nommat, nomgrp, nomen, prenomen, codensp, name, debut, fin, lieu, phone, joursEvent FROM events inner join matiere on codemp=codem inner join enseignant on matricule=codensp inner join contact on enseignant.matricule=contact.matricule  WHERE events.id not in(SELECT idevent FROM horairet where date_format(datet,\'%Y-%m-%d \')=:debuth) and date_format(debut,\'%Y-%m-%d \')=:debut and codensp=:code and events.promo=:promo order by(prenomen) LIMIT 10', array('debuth'=>date('Y-m-d'), 'debut'=>date('Y-m-d'), 'code'=>$_SESSION['search'], 'promo'=>$_SESSION['promo']));
                    }else{

                        $prodm=$DB->query('SELECT DISTINCT(codensp), events.id as id,  codemp, nommat, nomgrp, nomen, prenomen, codensp, name, debut, fin, lieu, phone, joursEvent FROM events inner join matiere on codemp=codem inner join enseignant on matricule=codensp inner join contact on enseignant.matricule=contact.matricule WHERE events.id not in(SELECT idevent FROM horairet where date_format(datet,\'%Y-%m-%d \')=:debuth) and date_format(debut,\'%Y-%m-%d \')=:debut and  events.promo=:promo order by(prenomen) LIMIT 10', array('debuth'=>date('Y-m-d'), 'debut'=>date('Y-m-d'), 'promo'=>$_SESSION['promo']));
                ...
                foreach ($prodm as $key => $value) {
                    ...
                    <form method="POST" action="eventlist.php" id="suitec" name="termc">
