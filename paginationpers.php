<?php
	...
	if (!empty($_SESSION['niveauf'])) {
 		...
 		$prodnbre =$DB->querys('SELECT count(*) as total from personnel  inner join niveau on personnel.numpers=niveau.matricule where niveau.nom=:niv order by(prenom)', array('niv'=>$_SESSION['niveauf']));

 	}else{

 		$prodnbre = $DB->querys('SELECT count(*) as total FROM personnel');


 	}


    	$lien='enseignant.php?personnel&effnav&page=';
