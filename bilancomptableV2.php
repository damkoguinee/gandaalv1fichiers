    		...
    		<div>
					...
					<tbody><?php 
						...
						<tr>
							...
							foreach ($panier->cursus() as $valuec) {
								...
								$resteapayerAbandons+=$rapport->bilanInscriptionApayerAbandons($valuec->nom, 'inscription', $promo)[0]+$rapport->bilanInscriptionApayerAbandons($valuec->nom, 'reinscription', $promo)[0]+$rapport->bilanFraiscolAbandons($valuec->nom, 'reinscription', $promo)[0];?><?php
				...
				<table class="payement">
					<tbody>
						...
						<tr><?php 
							$prev=$entreepaye+$resteapayer+$activitespaie+$versement;
							$entreepayeAbandons = ($rapport->fraisInscriptionAbandons($promo)['montant'] + $rapport->fraisScolariteAbandons($promo)['montant']);
							$prevAbandons=$entreepayeAbandons+$resteapayerAbandons;
						?>
							<th colspan="3" style="text-align:left;">Bilan des Entrées</th>

							<th colspan="2" style="text-align:center;">Effectif: <?=$effectif;?></th>

							<th colspan="3" style="text-align:center; background-color: orange;">Prév: <?=number_format($prev-$prevAbandons,0,',',' ');?></th>

							<th colspan="3" style="text-align:center; background-color: green;">Payé: <?=number_format($entreepaye+$activitespaie+$versement,0,',',' ');?></th>
 
							<th colspan="3" style="text-align:center; background-color: red;">Reste: <?=number_format($resteapayer-$prevAbandons,0,',',' ');?></th>

							<th colspan="4" style="text-align:center;;">TR: <?=number_format($tauxgeneral/5,0,',',' ');?>%</th>
						</tr>
						
						<tr>
							<th colspan="3" style="text-align:left;">Abandons</th>

							<th colspan="2" style="text-align:center;">Effectif: <?=$rapport->abandons($promo)['nbre'];?></th>

							<th colspan="3" style="text-align:center; background-color: orange;">Prév: <?=number_format($prevAbandons,0,',',' ');?></th>

							<th colspan="3" style="text-align:center; background-color: green;">Payé: <?=number_format($entreepayeAbandons,0,',',' ');?></th>
