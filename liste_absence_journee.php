    ...
    <div class="container-fluid">
		...
		<div class="col-sm-12 col-md-10"><?php
			...
			if (isset($_POST['justabs'])) {
				...
				$DB->insert('INSERT INTO just_absence_journee(id_absence, matricule, motif, datejust) values( ?, ?, ?, now())', array($id, $mat, $motif));?>
			...
			if (isset($_GET['supidabs'])) {
				...
				$DB->delete('DELETE FROM accesiteabsence WHERE id = ?', array($_GET['supidabs']));?>
			...
			if (isset($_POST['j1'])) {
                ...
                $prodeleve=$DB->query("SELECT *from inscription  inner join accesiteabsence on matricule = mat_abs where nomgr='{$_SESSION['classe']}' and promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteabsence.id not in(SELECT id_absence FROM just_absence_journee)  order by (mat_abs)");

			}elseif (isset($_GET['absmatri'])) {

				// $prodeleve=$DB->query('SELECT accesiteabsence.id as id, nomel, prenomel, eleve.adresse as adresse, eleve.sexe as sexe, pere, mere, eleve.naissance as naissance, eleve.matricule as matricule, phone, nommat, nomen, prenomen, nbreheure, hdebut, date_format(date_abs,\'%d/%m/%Y \') as date_abs, nomgr as classe, nomf from eleve inner join absence on eleve.matricule=absence.matricule inner join matiere on matiere.codem=absence.codem inner join enseignant on enseignant.matricule=absence.codens inner join contact on eleve.matricule=contact.matricule inner join formation on formation.codef=matiere.codef where promo=:promo and absence.matricule=:matr and semestre=:sem and accesiteabsence.id not in(SELECT id_absence FROM just_absence_journee)', array('promo'=>$_GET['promo'], 'matr' => $_GET['absmatri'], 'sem'=>$_GET['sem']));

                // $prodeleve=$DB->query("SELECT *from accesiteabsence inner join inscription on matricule = mat_abs where promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteabsence.id not in(SELECT id_absence FROM just_absence_journee)  order by (mat_abs)");

			}else{

				$prodeleve=$DB->query("SELECT *from inscription  inner join accesiteabsence  on matricule = mat_abs where promo='{$_SESSION['promo']}' and annee='{$_SESSION['promo']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") >= '{$_SESSION['date1']}' and DATE_FORMAT(date_abs, \"%Y%m%d\") <= '{$_SESSION['date2']}'  and accesiteabsence.id not in(SELECT id_absence FROM just_absence_journee)  order by (mat_abs)");
			} ?>
