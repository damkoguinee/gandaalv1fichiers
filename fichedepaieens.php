...
if (isset($_GET['payehemp'])) {
	...
	if (isset($_GET['matensind'])) {
		$products=$DB->query("SELECT *FROM enseignant where enseignant.matricule='{$_GET['matensind']}' ");
	}else{

		$products=$DB->query("SELECT *FROM enseignant "); 
	}  
    ...
    foreach ($products as $key => $product) {
    	...
    	$prodpaye=$DB->querys("SELECT id, numdec, matricule, mois, montant, heurep, typepaye, DATE_FORMAT(datepaye, \"%d/%m/%Y\")AS datepaye FROM payenseignant where mois='{$moisnum}' and matricule='{$matricule}' and anneescolaire='{$_SESSION['promo']}' ");
    	...
    	if (!empty($prodpaye['id'])) {
	    	...
	    	<page backtop="5mm" backleft="3mm" backright="1mm" backbottom="5mm"><?php
			    ...
			    $prodac=$DB->querys('SELECT *FROM accompte WHERE matricule= ? and moischaine=? and anneescolaire=?', array($matricule, $mois, $_SESSION['promo']));
			    ...
			    $prodsoc=$DB->querys('SELECT *FROM ssocialens WHERE numpers= ?', array($matricule));
			    ...
			    $prodpri=$DB->querys('SELECT *FROM prime WHERE numpersp= ? and promop=?', array($matricule, $_SESSION['promo']));
			    ...
			    <table style="margin-top: 30px; margin-left:30px; border-bottom: 0px;" class="border" >
			    	...
			    	<thead>
				        ...
				        <tr>
				          <th colspan="4">BULLETIN DE PAIE <?=strtoupper($mois);?> </th>
				        </tr>

				        <tr>
				          <th><?="Matricule:  " .$matricule; ?></th>
				          <th colspan="3" style="font-size: 18px;"><?=$panier->nomEnseignant($matricule); ?></th>
				        </tr>
			      	...
			      	<tbody><?php
				        ...
				        <tr>
				          ...
				          <td style="width: 10%;border:2px ; border-bottom: 0px;"><?=$prodpaye['heurep'];?></td>
