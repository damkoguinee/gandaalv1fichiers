    ...
    <div class="row"><?php  
        ...
        <table class="table table-hover table-bordered table-striped table-responsive text-center">
            ...
            if (isset($_POST['termec'])) {
              ...
              $prodm=$DB->query('SELECT enseignant.matricule as matricule, prenomen, nomen from enseignant inner join enseignantencours on enseignant.matricule=matriculens inner join salaireens on salaireens.numpers=enseignant.matricule where salaireens.promo LIKE ? and enseignantencours.promo LIKE ? and enseignant.matricule not in(SELECT matricule FROM payenseignant WHERE anneescolaire LIKE ? and mois LIKE ?) and (enseignant.matricule LIKE? or nomen LIKE ? or prenomen LIKE ?) order by(prenomen)',array($_SESSION['promo'], $_SESSION['promo'], $_SESSION['promo'], $_SESSION['moisp'], "%".$terme."%", "%".$terme."%", "%".$terme."%"));
                ...
                if (!empty($_SESSION['cursusp'])) {
                    ...
                    $prodm=$DB->query('SELECT enseignant.matricule as matricule, prenomen, nomen from enseignant inner join enseignantencours on enseignant.matricule=matriculens inner join salaireens on salaireens.numpers=enseignant.matricule where salaireens.promo=:promop and enseignantencours.promo=:promoc  and enseignant.matricule not in(SELECT matricule FROM payenseignant WHERE anneescolaire=:annee and mois=:mois) and salaireens.promo=:promo order by(prenomen)', array('promop'=>$_SESSION['promo'], 'annee'=>$_SESSION['promo'], 'mois'=>$_SESSION['moisp'], 'promoc'=>$_SESSION['promo'], 'promo'=>$_SESSION['promo']));
                }
