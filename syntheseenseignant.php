  ...
  <div class="row"><?php
    ...
    if (isset($_SESSION['pseudo'])) {
      ...
      if ($products['niveau']<4) {?>
        ...
        if ((isset($_GET['general']) or isset($_GET['sscol']) or isset($_POST['j1']) or isset($_POST['j2']) or isset($_GET['senseig']))) {?>
          ...
          <div class="col-sm-12 col-md-10">
            ...
            <table class="table table-bordered table-striped table-hover table-responsive text-center">
              ...
              <thead>
                <form class="form" method="POST" action="syntheseenseignant.php?sscol" name="termc">
                  ...
                  <th colspan="5">
                    <div class="row">
                      ...
                      <div class="col-sm-6 col-md-6">
                        <?='Paiements des enseignants '.$datenormale;?><a class="btn btn-warning" href="csv.php?enseig" target="_blank"><img  style="height: 20px; width: 20px;" src="css/img/excel.jpg"></a>
                      </div>
                    </div>
                  </th>
                </form>
              ...
              <tbody><?php
                ...
                if (isset($_POST['j1']) and isset($_POST['j2'])) {
                  ...
                  $proddec =$DB->query('SELECT enseignant.matricule as matricule, montant, DATE_FORMAT(datepaye, \'%d/%m/%Y\')AS datepaye, motif, heurep, nomen, prenomen FROM payenseignant inner join enseignant on enseignant.matricule=payenseignant.matricule WHERE anneescolaire=:promo and DATE_FORMAT(datepaye, \'%Y%m%d\') >= :date1 and DATE_FORMAT(datepaye, \'%Y%m%d\') <= :date2', array('promo' => $_SESSION['promo'], 'date1' => $_SESSION['date1'], 'date2' => $_SESSION['date2']));

                }else{

                  $proddec =$DB->query('SELECT enseignant.matricule as matricule, montant, DATE_FORMAT(datepaye, \'%d/%m/%Y\')AS datepaye, motif, heurep, nomen, prenomen FROM payenseignant inner join enseignant on enseignant.matricule=payenseignant.matricule WHERE anneescolaire=:promo', array('promo' => $_SESSION['promo'])); 
                }
                $totpaye=0;
                $toth=0;
                foreach( $proddec as $dec ){
                  $nom=strtoupper($dec->nomen).' '.ucwords($dec->prenomen);
                  $totpaye+=$dec->montant;
                  $toth+=$dec->heurep;?>

                  <tr><?php
                  if ($dec->motif!='payementpers') {?>
                  ...
                  <td class="text-end"><?= number_format($dec->montant,0,',',' ');?></td>

                  <td><?=$dec->datepaye;?></td><?php
                } ?>
