                        ...
                        if (isset($_POST['numel']) or isset($_GET['eleve']) or isset($_GET['delepaye']) or isset($_GET['annulins']) or isset($_GET['delins'])){?>
                            ...
                            <div style="display:flex;"><?php 
                                foreach ($prodtranche as $montranche) {
                                    ...
                                    <div style="color:white; font-size:16px; font-weight:bold;">Reste <?=$montranche->tranche;?> = <?=number_format(($montranche->montant*(1-($prodrem['remise']/100))-$payetranche),0,',',' ').'-';?></div><?php
                                }?>
                            ...
                            <ol style="margin-top: -10px;"><?php
                                ...
                                if (isset($_POST['numel']) AND empty($products)) {?>
                                    ...
                                    <li><label>Type de Réçu</label><select name="famille" required="" ><?php 
                                    ...
                                    <div style="display: flex;">
                                      <div style="width: 100%;">
                                        ...
                                        <li><label>Montant Payer*</label><input id="numberconvert" type="number"   name="mp" min="0" style="font-size: 25px; width: 40%;"></li>
                                      </div>
                            ...
                            if ($mois=='annuel') {
                                ...
                                foreach ($panier->tranche() as $valuet) {
                                    ...
                                    if (empty($prodrep)) {
                                        ...
                                        if ($_POST['famille']!='simple') {
                                            ...
                                            if (empty($cumul)) {
                                                $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, devise, taux, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($famille, $numel, $montantannuel, $devise, $taux, $mois, $typep, $bordereau, $banque, $promo, $famille));
                                            }else{
                                            ...
                                            $DB->insert('INSERT INTO payementfraiscol(numpaye, matricule, montant, tranche, famille, typepaye, numpaie, banque, promo, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($numpaye, $numel, $montantannuel, $mois, $famille, $_POST['typep'], $bordereau, $banque, $_POST['promo']));

                                            $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, devise, taux, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($numpaye, $numel, $montantannuel, $devise, $taux, $mois, $typep, $bordereau, $banque, $promo, $famille));
                                        ...
                                        <div class="alertes">un paiement existe pour cet élève</div><?php 
                                    }
                                    // code...
                                }
                                ...
                                while ($t<4) {
                                    ...
                                    if (empty($prodrep)) {
                                        ...
                                        if (empty($cumul)) {
                                            $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, taux, devise, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($famille, $numel, $montantannuel, $mois, $typep, $bordereau, $banque, $promo, $famille));
                                        }else{
                                ...
                                while ($t<10) {
                                    ...
                                    if (empty($prodrep)) {
                                        ...
                                        $DB->insert('INSERT INTO payementfraiscol(numpaye, matricule, montant, tranche, famille, typepaye, numpaie, banque, promo, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($famille, $numel, $montantannuel, $mois, $famille, $_POST['typep'], $bordereau, $banque, $promo));

                                        $cumul = $DB->querys("SELECT matricule, famille, montant FROM histopayefrais where famille='{$famille}' and matricule='{$numel}' ");

                                        if (empty($cumul)) {
                                            $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, taux, devise, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($famille, $numel, $montantannuel, $taux, $devise, $mois, $typep, $bordereau, $banque, $_POST['promo'], $famille));
                                        }else{
                                ...
                                if (empty($prodrep)) {
                                    ...
                                    if ($_POST['famille']!='simple') {
                                        ...
                                        if (empty($cumul)) {
                                            $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, devise, taux, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($famille, $numel, $mp, $devise, $taux, $mois, $typep, $bordereau, $banque, $promo, $famille));
                                        }else{
                                        ...
                                        $DB->insert('INSERT INTO payementfraiscol(numpaye, matricule, montant, tranche, famille, typepaye, numpaie, banque, promo, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($numpaye, $numel, $_POST['mp'], $mois, $famille, $_POST['typep'], $bordereau, $banque, $_POST['promo']));

                                        $DB->insert('INSERT INTO histopayefrais(numpaye, matricule, montant, devise, taux, tranche, typepaye, numpaie, banque, promo, famille, datepaye) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())',array($numpaye, $numel, $mp, $devise, $taux, $mois, $typep, $bordereau, $banque, $promo, $famille));
                                    }
                                                    ...
                                                    <td><?php 
                                                        ...
                                                        if ($_SESSION['etab']=='Complexe Scolaire la Plume') {
                                                            ...
                                                            if ($prodlogin['type']=='secrétaire' or $prodlogin['type']=='comptable' or $prodlogin['type']=='admin')  {?>
                                                                ...
                                                                <a href="comptabilite.php?delepaye=<?=$paye->famille;?>&matr=<?=$numel;?>&tranche=<?=$paye->tranche;?>&montant=<?=$paye->montant;?>" onclick="return alerteS();"><input type="button" value="Annuler paiement" style="width: 100%; font-size: 16px; background-color: red; text-align: center; color: white; cursor: pointer"></a><?php 
                                                            }
                                                        }else{
                                            ...
                                            <td style="text-align: center;"><?php 
                                                ...
                                                if ($_SESSION['etab']=='Complexe Scolaire la Plume') {
                                                    ...
                                                    if ($prodlogin['type']=='secrétaire' or $prodlogin['type']=='comptable' or $prodlogin['type']=='admin')  {?>
                                                        ...
                                                        <a href="comptabilite.php?delins=<?=$numel;?>" onclick="return alerteSins();"><input type="button" value="Annuler paie" style="width: 100%; font-size: 16px; background-color: red; color: white; width: 95%; cursor: pointer"></a><?php 
                                                    }
                                                }else{
